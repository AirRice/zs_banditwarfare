...
function ENT:Explode(eHitEntity)
	...
	if eHitEntity:IsValid() and eHitEntity:IsPlayer() and eHitEntity:Team() ~= owner:Team() then
		eHitEntity:PoisonDamage(self.Damage or 8, owner, self)
		eHitEntity:AddLegDamage(self.Damage*0.25)
		local dice = math.random(10)
		if dice == 5 then eHitEntity:KnockDown(1) end
	end
