...
function ENT:DrawTranslucent()
	local drawowner = MySelf:IsValid() and MySelf:IsPlayer() and self:GetOwner():IsPlayer() and MySelf:Team() == self:GetOwner():Team() and (GAMEMODE.AlwaysShowNails or MySelf:KeyDown(IN_SPEED) or MySelf:TraceLine(256, MASK_SHOT).HitPos:Distance(self:GetPos()) <= 16)
	...
	if drawowner then
		...
		if displayowner == "" then
			...
			if deployer:IsValid() then
				...
				if not (deployer:Team() == TEAM_HUMAN or deployer:Team() == TEAM_BANDIT) or not deployer:Alive() then
					displayowner = "(사망) "..displayowner
					redname = true
				end
			end
		end
