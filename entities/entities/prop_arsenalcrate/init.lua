...
function ENT:OnTakeDamage(dmginfo)
	if not self:GetObjectOwner():IsPlayer() then return end
...
function ENT:Use(activator, caller)
	...
	if not self.NoTakeOwnership and not self:GetObjectOwner():IsValid() and activator:Alive() then
		self:SetObjectOwner(activator)
	end

	if gamemode.Call("PlayerCanPurchase", activator) then
		activator:SendLua("GAMEMODE:OpenPointsShop()")
	elseif self:GetObjectOwner():IsPlayer() and activator:Team() == self:GetObjectOwner():Team() and activator:Alive() then
